...
- `docker image remove $image_name/id` remove image, require all the container from that image is removed.
	...
	- [[Technical/DevOps/docker/cli#^b22ac4|prune]]
...
- `docker start $container_id/container_name` start created container with detach mode. ^1cc093
	- `-i` [[Technical/DevOps/docker/cli#^ae9c17| ref]]
	- `-a` [[Technical/DevOps/docker/cli#^5cf14f| ref]]
- `docker run $image_id/image_name` create a new container from image in attach mode.
	...
	-  `-d` run in detach mode also apply for [[Technical/DevOps/docker/cli#^1cc093|docker run]].
	- `-a` run in attach mode also apply for [[Technical/DevOps/docker/cli#^1cc093|docker run]]. ^5cf14f
