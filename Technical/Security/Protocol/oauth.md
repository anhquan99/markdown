...
- OAuth is a security standard where you give one application permission to access *your data* in another application. The steps to grant permission, or *consent*, are often referred to as *authorization* or even *delegated authorization*. You authorize one application to access your data, or use features in another application on your behalf, without giving them your password.
- OAuth 2.0 is designed only for _authorization_, for granting access to data and features from one application to another.
...
- The URL the authorization server will redirect the resource owner back to after granting permission to the client, which is sometimes referred to as the _Callback URL_.
