	...
	1. ## Obtain the data source
		- The first step is to specify the data source. The `from` clause comes first in order to introduce the data srouce `customers` and the range variable `cust`
		  ...
		  var query = from cust in customers
					  select cust;
		  ``````
		  - The range variable is like iteration variable in a `foreach` loop except that no actual iteration occurs in a query expression. Additional range variables can be introduced by `let`
...
- ### `IQeuryable<T>`
	...
	- 2 types of syntax
		...
		- #### Method-based
		  ...
		  ``````
# `Equijoins`
- Performs a join againts equality or matching columns values of the associated tables.
